<template>
  <div class="force-discard-view">
    ...
    <div class="force-discard-view__discards">
      ...
      <div v-for="resource in me.attributes.resources" :key="resource.id" class="force-discard-view__discards-field">
        ...
        <div class="force-discard-view__discards-field-counter">
          <button :disabled="resourcesToDiscard[resource.type] <= 0" @click="decrement(resource.type)">
            -
          </button>
          <input :id="resource.type" v-model="resourcesToDiscard[resource.type]" type="number" min="0" :max="resourcesToDiscard[resource.type]">
          <button :disabled="resourcesToDiscard[resource.type] >= resource.value" @click="increment(resource.type)">
            +
          </button>
        </div>
      </div>
    ...
    <div class="force-discard-view__finish">
      <button :disabled="amountToDiscard > 0" class="force-discard-view__finish-button" @click="sendClientResponse">
        {{ T("FINISH_DISCARD") }}
      </button>
    </div>
  </div>
